...
export function PlanAnnotations({ documentId, projectId, planUrl, lotId }: PlanAnnotationsProps) {
  ...
  return (
    <div className="space-y-2">
      <div
        ref={containerRef}
        className="relative h-[480px] w-full cursor-crosshair overflow-hidden rounded-2xl border border-gray-300 bg-gray-900 dark:border-gray-600"
        onClick={handleClick}
      >
        <img
          src={planUrl}
        ...
        {annotations.map((annotation) => (
          ...
          >
            <div className="relative">
              <div className="flex h-6 w-6 items-center justify-center rounded-full bg-amber-500 text-xs font-bold text-white shadow-lg ring-2 ring-white dark:ring-gray-900">
                <MessageSquare className="h-3 w-3" />
              </div>

              {selectedAnnotation === annotation.id && (
                <div className="absolute left-8 top-0 z-10 w-64 rounded-lg border border-gray-200 bg-white p-3 shadow-xl dark:border-gray-700 dark:bg-gray-800">
                  <div className="flex items-start justify-between">
                    ...
                    <button
                      ...
                      className="ml-2 rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-red-600 dark:hover:bg-gray-700"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
