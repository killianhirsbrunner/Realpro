      ...
      <div>
        ...
        <div className="space-y-3">
          {groupedTasks.pending.map((task) => {
            ...
            return (
              <div
                ...
                className="p-4 bg-white border border-neutral-200 rounded-xl hover:shadow-md transition-shadow cursor-pointer"
              >
                <div className="flex items-start gap-3">
                  <button
                    ...
                    className="mt-0.5 text-neutral-400 hover:text-brand-600"
                  >
          ...
          {groupedTasks.pending.length === 0 && (
            <p className="text-neutral-500 text-sm py-4">Aucune tâche en cours</p>
          )}
        </div>
      </div>
        ...
        <div className="space-y-3">
          {groupedTasks.completed.slice(0, 5).map((task) => (
            <div
              key={task.id}
              onClick={() => onTaskClick?.(task.id)}
              className="p-4 bg-neutral-50 border border-neutral-200 rounded-xl opacity-75 hover:opacity-100 transition-opacity cursor-pointer"
            >
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-600 mt-0.5" />
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-neutral-700 line-through">{task.title}</p>
                  {task.assignee && (
                    <p className="text-sm text-neutral-500 mt-1">{task.assignee}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {groupedTasks.completed.length === 0 && (
            <p className="text-neutral-500 text-sm py-4">Aucune tâche terminée</p>
          )}
        </div>
      </div>
    </div>
