...
import { AlertCircle, ArrowRight, Check, Clock, Building2, Sparkles } from 'lucide-react';
...
export function RegisterDemo() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        // 2. Récupérer le plan Demo
        const { data: demoPlan, error: planError } = await supabase
          .from('plans')
          .select('*')
          .eq('slug', 'demo')
          .eq('is_active', true)
        ...
        if (!demoPlan) {
          throw new Error('Le plan Demo n\'est pas disponible. Veuillez contacter le support.');
        }

        // 3. Créer l'organisation
        const companySlug = formData.company.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        const { data: orgData, error: orgError } = await supabase
          .from('organizations')
          .insert({
            name: formData.company || `Demo ${formData.firstName}`,
            slug: `${companySlug}-demo-${Date.now()}`,
            default_language: 'FR',
            is_active: true
          })
          .select()
        ...
        // 4. Lier l'utilisateur à l'organisation
        ...
        // 5. Créer la souscription Demo avec période de 14 jours
      ...
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-neutral-900 dark:text-neutral-100 mb-2">
            Essayez RealPro gratuitement
          </h1>
          <p className="text-neutral-600 dark:text-neutral-400">
            Découvrez toutes les fonctionnalités pendant 14 jours
          </p>
        </div>

        {/* Avantages du compte démo */}
        <div className="bg-white/60 dark:bg-neutral-900/60 backdrop-blur-xl rounded-xl border border-neutral-200 dark:border-neutral-800 p-4 mb-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="flex flex-col items-center">
              <div className="w-10 h-10 rounded-lg bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center mb-2">
                <Clock className="w-5 h-5 text-brand-600 dark:text-brand-400" />
              </div>
              <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100">14 jours</span>
              <span className="text-xs text-neutral-500">d'accès gratuit</span>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-10 h-10 rounded-lg bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center mb-2">
                <Building2 className="w-5 h-5 text-brand-600 dark:text-brand-400" />
              </div>
              <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100">1 projet</span>
              <span className="text-xs text-neutral-500">de démonstration</span>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-10 h-10 rounded-lg bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center mb-2">
                <Check className="w-5 h-5 text-brand-600 dark:text-brand-400" />
              </div>
              <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Sans CB</span>
              <span className="text-xs text-neutral-500">aucun paiement</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 dark:bg-neutral-900/80 backdrop-blur-xl rounded-2xl border border-neutral-200 dark:border-neutral-800 p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                Entreprise <span className="text-neutral-400">(optionnel)</span>
              </label>
              <Input
        ...
        <div className="text-center mt-6 space-y-3">
          ...
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            Vous souhaitez un plan complet ?{' '}
            <Link to="/auth/register" className="text-brand-600 dark:text-brand-400 font-semibold hover:text-brand-700 dark:hover:text-brand-300 transition-colors">
              Voir les offres
            </Link>
          </p>
        </div>
